...
def main():
    ...
    if args.output:
        ...
        print(f"Wrote {len(results)} pair(s) to {out_path}")
        for r in results:
            g = r["grades"]
            level = "AAA" if g["AAA_normal"] else "AA" if g["AA_normal"] else \
                "AA large only" if g["AA_large"] else "fails AA"
            label = r["label"] or f"{r['fg']} on {r['bg']}"
            print(f"  {label}: {r['ratio']}:1 {level}")
    else:
        print(output)
