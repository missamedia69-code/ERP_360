...
object ModulesSocle {
    ...
    /** Dépendances métier incompressibles : tu achètes ou tu produis → tu stockes. */
    val dependances: Map<ModuleCode, Set<ModuleCode>> = mapOf(
        ModuleCode.ACH to setOf(ModuleCode.STK),
        ModuleCode.PRO to setOf(ModuleCode.STK),
    )

    /**
     * Ajoute transitivement les dépendances (ACH→STK, PRO→STK, et VEN→STK si
     * [autoriserVenteSansStock] est faux).
     */
    fun avecDependances(
        modules: Collection<ModuleCode>,
        autoriserVenteSansStock: Boolean = false,
    ): Set<ModuleCode> {
        val resultat = linkedSetOf<ModuleCode>()
        resultat.addAll(modules)
        var ajoute: Boolean
        do {
            ajoute = false
            val aAjouter = mutableSetOf<ModuleCode>()
            for (m in resultat.toList()) {
                dependances[m]?.let { deps -> aAjouter.addAll(deps.filter { it !in resultat }) }
                if (!autoriserVenteSansStock && m == ModuleCode.VEN && ModuleCode.STK !in resultat && ModuleCode.STK !in aAjouter) {
                    if (resultat.any { it == ModuleCode.ACH || it == ModuleCode.PRO }) {
                        aAjouter.add(ModuleCode.STK)
                    }
                }
            }
            if (aAjouter.isNotEmpty()) {
                resultat.addAll(aAjouter)
                ajoute = true
            }
        } while (ajoute)
        return resultat
     ...
     * Les dépendances (ACH→STK, PRO→STK) sont ajoutées automatiquement.
     */
    fun metierActifs(
        ...
        val avecDeps = avecDependances(brut, autoriserVenteSansStock)
        return metier.filter { it in avecDeps }
    }
