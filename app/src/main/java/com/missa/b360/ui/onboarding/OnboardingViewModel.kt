...
class OnboardingViewModel @Inject constructor(
    ...
    private fun restaurerProgression() {
        viewModelScope.launch {
            runCatching { getOnboardingProgress() }.getOrNull()?.let { progression ->
                ...
                venteSansStock = settingsStore.get(SettingsStore.Keys.VENTE_SANS_STOCK) == "true"
    ...
    fun choisirProfil(p: ProfilActivite) {
        ...
        if (p != ProfilActivite.CUSTOM) {
            ...
            val pack = ModulesSocle.avecDependances(packBrut, venteSansStock)
            modulesPersonnalises = if (changementDeProfil) pack
            else ModulesSocle.avecDependances(modulesPersonnalises + packBrut, venteSansStock).toSet()
        }
        rafraichirSocle()
        enregistrerModules()
    }
    ...
    fun modulesMetier(): List<ModuleCode> =
        ModulesSocle.metierActifs(profil, modulesPersonnalises, venteSansStock)
